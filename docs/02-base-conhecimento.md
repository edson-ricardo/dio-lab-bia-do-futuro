...
| Arquivo | Formato | Para que serve o Rico? |
|---------|---------|---------------------|
...
O produto Fundo Imobiliário (FII) substitui o Fundo Multimercado, pois pessoalmente me sinto mais confiante em usar apenas produtos financeiros que eu conheço. Assim, poderei validar as respostas do Rico de forma mais assertiva.
...
2. Carregar os arquivos via código, como no exemplo abaixo:
...
#JSONs
with open('data/perfil_investidor.json', 'r', encoding='fut-8') as f:
  perfil = json.load(f)

with open('data/produtos_financeiros.json', 'r', encoding='fut-8') as f:
